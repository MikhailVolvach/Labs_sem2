...
void testing(string dbName, string tabName) {
	...
	dbmsLib::ReadDBTable5(tab, path + tabName + ".txt");
	dbmsLib::PrintTable5(tab, screenWidth);
...
int main() {
	...
	while (true) {
		...
		case 1: 
			...
			break;
		case 2: 
			//dbmsLib::PrintTable5(tab, screenWidth);//Печать таблицы БД (screenWidth-ширина экрана)
			tab.PrintTable(screenWidth);
			break;
		case 3: 
			//tab.WriteDBTable(path + tabName + ".txt");
			tabName += "_copy";
			dbmsLib::WriteDBTable5(tab, path + tabName + ".txt");
			break;
		case 4: 
